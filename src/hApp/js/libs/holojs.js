...
<<<<<<< HEAD
    ...
    //  payload = this.preparePayload(zome,fn,payload);
      connect({ url: "ws://localhost:3000" }).then(async({callZome, close}) => {

        if(typeof callback == 'function') {
            callZome(this.instance_name, zome ,fn)(payload).then((r)=>{
              console.log("RESULT : ",r);
              callback(r);
            });
        } else {
          //  response = await xhr.post(this.conductor_endpoint).set('Content-Type', 'application/json').set('accept', 'json').send(payload);
          await callZome(this.instance_name, zome ,fn)(payload).then(
                  (r)=>{
                    try {
                      console.log("RESULT : ",fn,payload,r);
                    return JSON.parse(r).Ok
